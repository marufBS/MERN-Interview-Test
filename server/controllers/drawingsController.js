...
export const updateDrawing = async (req, res) => {
    try {
        const { id } = req.params
        const {drawingTitle, canvas} = req.body
        console.log(drawingTitle,canvas)
        const updateDrawing = await drawingModel.findOneAndUpdate({ _id: id }, {id, canvas, drawingTitle }, { new: true, upsert: true })
